...
echo $name
curl -ks --user $user:$token https://$url/api/blueprints/$id | jq '.results | { id, name, version, tags, description, shortDescription, externalLink, imageLink, yml, visibility, params, entitlementType, entitledUsers, entitledUserGroups }' > "HCP-$name.$(date +%F_%R).txt"
